...
/** Save a player's profile photo (base64 string). */
export async function savePhoto(poolId, playerName, base64) {
  try {
    await set(ref(db, `pools/${poolId}/photos/${playerName}`), base64);
    return true;
  } catch (e) {
...
/** Save tournament-specific data within a pool. */
export async function saveTournament(poolId, tournamentId, data) {
  try {
    await set(ref(db, `pools/${poolId}/tournaments/${tournamentId}`), data);
  } catch (e) {
    console.error("Tournament save error:", e);
  }
}

/** Update specific fields of a tournament within a pool. */
export async function updateTournament(poolId, tournamentId, updates) {
  try {
    await update(ref(db, `pools/${poolId}/tournaments/${tournamentId}`), updates);
  } catch (e) {
    console.error("Tournament update error:", e);
  }
}

/** Claim admin role for a pool (requires correct join code). */
export async function claimAdmin(poolId, uid, providedJoinCode) {
  try {
    const snap = await get(ref(db, `pools/${poolId}/joinCode`));
    if (!snap.exists() || snap.val() !== providedJoinCode) {
      return { ok: false, error: "Invalid pool code" };
    }
    await update(ref(db, `pools/${poolId}`), { adminUid: uid });
    return { ok: true };
  } catch (e) {
